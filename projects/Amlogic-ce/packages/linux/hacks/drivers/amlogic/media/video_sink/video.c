...
int hack_novsync;
MODULE_PARM_DESC(hack_novsync, "\n hack_novsync\n");
module_param(hack_novsync, int, 0664);
...
static u64 dbg_vsync_cnt;
static u64 dbg_peek_ok_cnt;
static u64 dbg_peek_null_cnt;

static u64 dbg_switch_cnt;
static u64 dbg_put_cnt;
...
static inline struct vframe_s *video_vf_get(void)
{
	...
	if (vf)
	{
		...
		video_notify_flag |= VIDEO_NOTIFY_PROVIDER_GET;
		if ((vf->type & VIDTYPE_MVC) && (framepacking_support) && (framepacking_width) && (framepacking_height))
		{
		...
		if (((process_3d_type & MODE_FORCE_3D_TO_2D_LR) || (process_3d_type & MODE_FORCE_3D_LR) || (process_3d_type & MODE_FORCE_3D_FA_LR)) && (!(vf->type & VIDTYPE_MVC)) && (vf->trans_fmt != TVIN_TFMT_3D_FP))
		{
		...
		if (((process_3d_type & MODE_FORCE_3D_TO_2D_TB) || (process_3d_type & MODE_FORCE_3D_TB) || (process_3d_type & MODE_FORCE_3D_FA_TB)) && (!(vf->type & VIDTYPE_MVC)) && (vf->trans_fmt != TVIN_TFMT_3D_FP))
		{
...
static void process_hdmi_video_sync(struct vframe_s *vf)
{
	...
	if ((!hdmi_delay_first_check && !hdmi_delay_normal_check &&
		 hdmin_delay_start == 0) ||
		!vf || last_required_total_delay <= 0)
		return;
...
static struct vframe_s *vsync_toggle_frame(struct vframe_s *vf, int line)
{
	...
	if ((vf != &vf_local) && (vf) && !vsync_pts_aligned)
	{
		...
		{
			vsync_pts_align = 0; /*vsync_pts_inc / 4 -
	...
	if (*expired)
	{
		if (pts_trace < expected_curr_interval)
		{
			...
			if (/*((int)(nextPts + expected_prev_interval * */
				...
				((int)(nextPts + (expected_prev_interval + 1) * vsync_pts_inc - next_vf_nextpts) >= 0))
			{
				*expired = false;
				if (pts_log_enable[PTS_32_PATTERN] || pts_log_enable[PTS_22_PATTERN])
	...
	else
	{
		if (pts_trace == expected_curr_interval)
		{
			...
			if (((int)(nextPts + vsync_pts_inc - next_vf->pts) >= 0) &&
				((int)(nextPts +
					   vsync_pts_inc * (expected_prev_interval - 1) - next_vf_nextpts) < 0) &&
				((int)(nextPts + expected_prev_interval * vsync_pts_inc - next_vf_nextpts) >= 0))
			{
				*expired = true;
				if (pts_log_enable[PTS_32_PATTERN] || pts_log_enable[PTS_22_PATTERN])
							   ...
							   int toggled_cnt)
{
	...
	{
		if (((atomic_read(&trickmode_framedone) == 0) || (trickmode_i == 1)) && (!to_notify_trick_wait) && (trickmode_duration_count <= 0))
		{
#if 0
			if (cur_vf)
...
#endif
	if (((pts == 0) && ((cur_dispbuf && (cur_dispbuf != &vf_local)) || (hold_property_changed == 1))) || (freerun_mode == FREERUN_DUR))
	{
		pts =
			timestamp_vpts_get() +
		...
		((next_vf->flag & VFRAME_FLAG_NO_DISCONTINUE) == 0))
	{
			 ...
			 ((int)(systime + vsync_pts_align - pts) >= 0)))
		{
			...
			if (systime > next_vf->pts || next_vf->pts == 0 ||
				...
				return true;
			if (omx_secret_mode == true && cur_omx_index >= next_vf->omx_index)
				return true;

			return false;
		}
		else if (omx_secret_mode == true && cur_omx_index >= next_vf->omx_index)
		{
			return true;
		}
	...
	else if (omx_run && omx_secret_mode && (omx_pts + omx_pts_interval_upper < next_vf->pts) && (omx_pts_set_index >= next_vf->omx_index))
	{
	...
	if (smooth_sync_enable)
	{
		...
		if ((abs(org_vpts + vsync_pts_inc + video_delay_val - systime) <
			 M_PTS_SMOOTH_MAX) &&
			(abs(org_vpts + vsync_pts_inc + video_delay_val - systime) > M_PTS_SMOOTH_MIN))
		{
	...
	if (tsync_get_mode() == TSYNC_MODE_PCRMASTER)
		expired = (timestamp_pcrscr_get() /*+ vsync_pts_align*/ >= pts) ? true : false;
	else
		expired = ((int)(timestamp_pcrscr_get() +
...
static int dvel_swap_frame(struct vframe_s *vf)
{
	...
	else if (vf)
	{
		...
		u32 new_dvel_w = (vf->type & VIDTYPE_COMPRESS) ? vf->compWidth : vf->width;
...
/* TODO: check if need */
		...
		if (cur_frame_par)
		{
			...
			if (cur_frame_par->VPP_hd_start_lines_ >= cur_frame_par->VPP_hd_end_lines_)
				h_size = 0;
			else
				h_size = cur_frame_par->VPP_hd_end_lines_ - cur_frame_par->VPP_hd_start_lines_ + 1;
			h_size /= (cur_frame_par->hscale_skip_count + 1);
			if (cur_frame_par->VPP_vd_start_lines_ >= cur_frame_par->VPP_vd_end_lines_)
				v_size = 0;
			else
				v_size = cur_frame_par->VPP_vd_end_lines_ - cur_frame_par->VPP_vd_start_lines_ + 1;
			v_size /=
		...
		{
			h_size = (disp_vf->type & VIDTYPE_COMPRESS) ? disp_vf->compWidth : disp_vf->width;
			v_size = (disp_vf->type & VIDTYPE_COMPRESS) ? disp_vf->compHeight : disp_vf->height;
			frame_size = (h_size << 16) | v_size;
		}
									...
									unsigned int height, bool force_adjust)
{
	...
	if ((width >= 2000) && (height >= 1400) &&
		(((dmc_config_state != 1) && (toggle_same_count < 30)) || force_adjust))
	{
		if (0)
		{ /* if (is_dolby_vision_enable()) { */
			/* vpu dmc */
...
void set_hdr_to_frame(struct vframe_s *vf)
{
	...
	if (has_hdr_info)
	{
		...
		vf->signal_type = (1 << 29) | (5 << 26) /* unspecified */
						  | (0 << 25)			/* limit */
	...
	struct vframe_s *vf1, int line)
{
	...
	if (debug_flag & DEBUG_FLAG_PRINT_DISBUF_PER_VSYNC)
		pr_info("swap frame to screen:%p\n", vf1);
	if (!vf1)
		return;
...
#endif
{
	...
	if (hack_novsync)
	{
		...
		{
			if (debug_flag & DEBUG_FLAG_PRINT_DISBUF_PER_VSYNC)
				pr_err("hack_novsync video_vf_get_states error\n");
			// return IRQ_HANDLED;
		}
		else
		{
			if (debug_flag & DEBUG_FLAG_PRINT_DISBUF_PER_VSYNC)
				pr_err("hack_novsync error buf_avail_num=%d\n", states.buf_avail_num);
		}
		...
		if (video_vf_get_states(&states) == 0 &&
			states.buf_avail_num > 1)
		{

			int drop = states.buf_avail_num - 1;

			if (debug_flag & DEBUG_FLAG_PRINT_DISBUF_PER_VSYNC)
				pr_info("hack_novsync drop %d\n", drop);
			while (drop-- > 0)
			{
				struct vframe_s *old = video_vf_get();
			...
			// if ((dbg_vsync_cnt % 300) == 0)
			//{ /* 60Hz 大约 5 秒；120Hz 大约 2.5 秒 */
			//	pr_info("peek_stat: vsync=%llu ok=%llu null=%llu,switch=%llu put=%llu\n",
			//			dbg_vsync_cnt, dbg_peek_ok_cnt, dbg_peek_null_cnt, dbg_switch_cnt, dbg_put_cnt);
			// }
		}
	}
	...
	if (is_dolby_vision_enable() && dovi_drop_flag)
	{
		...
		while (cnt--)
		{
			...
			{
				max_drop_index = omx_run ? omx_need_drop_frame_num : dovi_drop_frame_num;
...
#ifdef CONFIG_AMLOGIC_MEDIA_ENHANCEMENT_DOLBYVISION
			if (is_dolby_vision_enable() && vf && is_dovi_frame(vf))
			{
				break;
			}
#endif
	...
	if (vf)
	{
		...
		{
			if ((vf->sidebind_type == glayer_info[0].sideband_type) || (vf->sidebind_type == 0))
			{
		...
		if (buf_cnt > 2)
		{
			...
			if (video_info->sync_duration_num > 0)
				hdmin_delay_max_ms = 1000 *
									 video_info->sync_duration_den /
									 video_info->sync_duration_num * (buf_cnt - 2);
		}
	}
	...
	if (is_meson_txlx_cpu() && dmc_adjust)
	{
		...
		if (chk_vf)
		{
			...
			else if ((chk_vf->type & VIDTYPE_COMPRESS) && cur_frame_par && cur_frame_par->vscale_skip_count)
			{
	...
	else
	{
		...
		{
			u32 inc = (vsync_slow_factor / 1000) * vsync_pts_inc / 1000;
	...
	if (omx_secret_mode == true)
	{
		...
		if (time_setomxpts > 0 && time_setomxpts_last > 0)
		{
			...
			if ((time_setomxpts_delta >
				...
				((diff - omx_pts_interval_upper * 3 / 2) > 0) || ((diff - omx_pts_interval_lower * 3 / 2) < 0))
			{
				time_setomxpts = 0;
		...
		if (((diff - omx_pts_interval_upper) > 0 || (diff - omx_pts_interval_lower) < 0 || (omx_pts_set_from_hwc_count < OMX_MAX_COUNT_RESET_SYSTEMTIME)) && video_start_post)
		{
		...
		else if (omx_pts_set_from_hwc_count == OMX_MAX_COUNT_RESET_SYSTEMTIME)
		{
			timestamp_pcrscr_set(omx_pts + DURATION_GCD);
		...
		else if (((diff - omx_pts_interval_upper / 2) > 0 || (diff - omx_pts_interval_lower / 2) < 0) && (omx_pts_set_from_hwc_count_begin < OMX_MAX_COUNT_RESET_SYSTEMTIME_BEGIN) && video_start_post)
		{
			timestamp_pcrscr_enable(1);
			if (debug_flag & DEBUG_FLAG_PTS_TRACE)
		...
		else if (is_dolby_vision_enable() && ((diff - omx_pts_dv_upper) > 0 || (diff - omx_pts_dv_lower) < 0) && video_start_post)
		{
			timestamp_pcrscr_set(omx_pts + DURATION_GCD);
		}
	}
	...
	/* hack_novsync: always show最新帧，丢掉队列中的老帧 */
	...
	if (hack_novsync)
	{
		/* 0) 可选：只在 hack_novsync 或 show_nosync 场景启用 */
		/* if (!hack_novsync) goto legacy_path; */

		/* 1) 先把 provider 队列压缩成 1 帧（保留你现有 hack_novsync 清队列逻辑即可） */

		/* 2) 只处理 1 帧：peek -> get -> toggle -> 结束 */
		vf = video_vf_peek();
		if (vf)
		{
			vf = video_vf_get();
...
#ifdef CONFIG_AMLOGIC_MEDIA_ENHANCEMENT_DOLBYVISION
					/* 低延迟模式建议：不跑 DV toggle，或者跑但不 wait（看你需求） */
					/* dv_new_vf = dvel_toggle_frame(vf, true); */
#endif
					if (!path0_new_frame || !cur_dispbuf)
					{
						/* 极简低延迟兜底：强制设置当前显示帧 */
						cur_dispbuf = vf;
					}
		...
		/* 3) 直接跳到函数尾部，不走下面复杂 while */
		goto after_vf_process;
	}
	...
	while (vf && !video_suspend)
	{
		...
		if ((omx_continuous_drop_flag && omx_run) && !(debug_flag & DEBUG_FLAG_OMX_DISABLE_DROP_FRAME))
		{
#ifdef CONFIG_AMLOGIC_MEDIA_ENHANCEMENT_DOLBYVISION
			if (is_dolby_vision_enable() && vf &&
				is_dovi_frame(vf))
		...
		if (vpts_expire(cur_dispbuf, vf, toggle_cnt) || show_nosync)
		{
			...
			if (debug_flag & DEBUG_FLAG_PTS_TRACE)
			{
				...
				if (pts_trace > 4 || pts_trace == 0)
					pr_info("exception:pts trace:%d.", pts_trace);
				if (vf->pts && vf->pts < timestamp_vpts_get())
				{
					...
					t = t / 90;
					pr_info("exception:video jumpback %dms.", t);
				}
			}
		...
		else
		{
			...
			{
				ATRACE_COUNTER(MODULE_NAME, __LINE__);
				if (duration_expire(cur_dispbuf, vf,
...
after_vf_process:
		...
		(gvideo_recv[0]->path_id == vd1_path_id))
	{
		...
		if (!new_frame)
		{
			...
			else if (vd_layer[0].dispbuf != gvideo_recv[0]->cur_buf)
			{
			 ...
			 (gvideo_recv[1]->path_id == vd1_path_id))
	{
		...
		if (!new_frame)
		{
			...
			else if (vd_layer[0].dispbuf != gvideo_recv[1]->cur_buf)
			{
	...
	else if (vd1_path_id == VFM_PATH_PIP)
	{
		...
		if (!new_frame)
		{
			...
			else if (vd_layer[0].dispbuf != cur_pipbuf)
			{
			 ...
			 (vd1_path_id != VFM_PATH_AUTO))
	{
		...
		if (!new_frame)
		{
			...
			else if (vd_layer[0].dispbuf != cur_dispbuf)
			{
	...
	amvecm_on_vs(
		...
		cur_frame_par ? cur_frame_par->supsc1_hori_ratio : 0,
		cur_frame_par ? cur_frame_par->supsc1_vert_ratio : 0,
		cur_frame_par ? cur_frame_par->spsc1_w_in : 0,
		cur_frame_par ? cur_frame_par->spsc1_h_in : 0,
		cur_frame_par ? cur_frame_par->cm_input_w : 0,
		cur_frame_par ? cur_frame_par->cm_input_h : 0,
		VD1_PATH);
#endif
		...
		(gvideo_recv[0]->path_id == vd2_path_id))
	{
		...
		if (!new_frame2)
		{
			...
			else if (vd_layer[1].dispbuf != gvideo_recv[0]->cur_buf)
			{
				/* video_render.0 has frame in display */
			 ...
			 (gvideo_recv[1]->path_id == vd2_path_id))
	{
		...
		if (!new_frame2)
		{
			...
			else if (vd_layer[1].dispbuf != gvideo_recv[1]->cur_buf)
			{
				/* video_render.1 has frame in display */
	...
	else if (vd2_path_id == VFM_PATH_AMVIDEO)
	{
		...
		if (!new_frame2)
		{
			...
			else if (vd_layer[1].dispbuf != cur_dispbuf)
			{
	...
	else if (vd2_path_id != VFM_PATH_INVAILD)
	{
		...
		if (!new_frame2)
		{
			...
			else if (vd_layer[1].dispbuf != cur_pipbuf)
			{
	...
	amvecm_on_vs(
		...
		curpip_frame_par ? curpip_frame_par->supsc1_hori_ratio : 0,
		curpip_frame_par ? curpip_frame_par->supsc1_vert_ratio : 0,
		curpip_frame_par ? curpip_frame_par->spsc1_w_in : 0,
		curpip_frame_par ? curpip_frame_par->spsc1_h_in : 0,
		curpip_frame_par ? curpip_frame_par->cm_input_w : 0,
		curpip_frame_par ? curpip_frame_par->cm_input_h : 0,
		VD2_PATH);
#endif
		...
		if (cur_frame_par)
			di_post_vf->process_fun(
				...
				vd_layer[0].start_x_lines |
					(cur_frame_par->vscale_skip_count << 24) | (frame_par_di_set << 16),
...
exit:
	// if (cur_dispbuf != last_disp_vf)
	//{
	//	dbg_disp_switch_cnt++;
	//	last_disp_vf = cur_dispbuf;
	// }
#ifdef CONFIG_AMLOGIC_MEDIA_DEINTERLACE
	if (legacy_vpp &&
			...
			if (vf->src_fmt.md_buf && vf->src_fmt.comp_buf)
			{
				if (debug_flag & DEBUG_FLAG_OMX_DV_DROP_FRAME)
					pr_info("parse vf %p, sei %p, size %d, md_buf %p\n",
							vf, sei, size,
							vf->src_fmt.md_buf);
				ret = parse_sei_and_meta_ext(vf, sei, size,
...
static int free_layer(u32 layer_id)
{
	...
	if (layer_id == 0)
	{
		if ((layer_cap & LAYER0_BUSY) && (layer_cap & LAYER0_AVAIL))
		{
			ret = 0;
			layer_cap &= ~LAYER0_BUSY;
		}
	...
	{
		if ((layer_cap & LAYER1_BUSY) && (layer_cap & LAYER1_AVAIL))
		{
			ret = 0;
			layer_cap &= ~LAYER1_BUSY;
		}
...
static ssize_t hack_novsync_show(struct class *cla,
								 struct class_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", hack_novsync);
}

static ssize_t hack_novsync_store(struct class *cla,
								  struct class_attribute *attr,
								  const char *buf, size_t count)
{
	int r;

	r = kstrtoint(buf, 0, &hack_novsync);
	if (r < 0)
		return -EINVAL;

	pr_info("hack_novsync=%d\n", hack_novsync);
	return count;
}
								  ...
								  struct class_attribute *attr, char *buf)
{
	...
	if (dispbuf)
	{
		u32 adapted_mode = (dispbuf->ratio_control & DISP_RATIO_ADAPTED_PICMODE) ? 1 : 0;
		u32 info_frame = (dispbuf->ratio_control & DISP_RATIO_INFOFRAME_AVAIL) ? 1 : 0;
...
#define XVYCC_VD1_RGB_CTRST (0x3170)
	...
	if (cur_dispbuf && (cur_dispbuf != &vf_local) && (cur_dispbuf != &hist_test_vf))
		pat_val = 0;
	...
	if (pat_val > 0 && pat_val <= 0x3fffffff)
	{
		if (!hist_test_flag)
		{
					...
					canvas_width * frame_height * 3))
			{
				...
				{
					if (ginfo->width >
						(layer->layer_width + layer->layer_left))
						hist_dst_w =
							layer->layer_width + layer->layer_left;
					else
						hist_dst_w =
					...
					if (ginfo->field_height >
						(layer->layer_height + layer->layer_top))
						hist_dst_h =
							layer->layer_height + layer->layer_top;
					else
						hist_dst_h =
	...
	const char *buf, size_t count)
{
	...
	if ((mode < VIDEO_WIDEOPTION_MAX) && (mode != layer->wide_mode))
	{
		layer->wide_mode = mode;
		vd_layer[1].property_changed = true;
	}
	...
	void *para)
{
	...
	case AMVIDEO_UPDATE_OSD_MODE:
		...
		val = osd_vpp_misc & (~osd_vpp_misc_mask);
	...
	case AMVIDEO_UPDATE_SIGNAL_MODE:
		...
		vd_signal_notifier_call_chain(VIDEO_SIGNAL_TYPE_CHANGED,
									  &vd_signal);
		break;
	default:
		break;
