...
#ifndef CONFIG_AMLOGIC_M8B_MESON32_VERSION
...
static u32 hacked_lowlatency = 1;
/*data_resend_policy:
...
static struct BuffInfo_s amvh265_workbuff_spec[WORK_BUF_SPEC_NUM] = {
	...
	{.max_width = 4096, .max_height = 2048, .ipp = {
												/* IPP work space calculation :
												 *   4096 * (Y+CbCr+Flags) = 12k, round to 16k
												 */
												.buf_size = 0x4000,
...
#ifdef MV_USE_FIXED_BUF
	 .mpred_mv = {
		 /*checked*/ /* 1080p, 0x40000 per buffer */
						   ...
						   struct BuffInfo_s *buf_spec)
{
	...
	if (hevc && get_dbg_flag2(hevc))
	{
		...
		if ((get_dbg_flag2(hevc) &
			 H265_DEBUG_SEND_PARAM_WITH_REG) == 0)
		{
			hevc_print(hevc, 0,
					   "rpm.buf_start             :%x\n",
					   buf_spec->rpm.buf_start);
...
{
	int max = (get_cpu_major_id() >= AM_MESON_CPU_MAJOR_ID_SM1) ? MAX_SIZE_8K : MAX_SIZE_4K;
...
static unsigned char is_skip_decoding(struct hevc_state_s *hevc,
									  struct PIC_s *pic)
{
	if (pic->error_mark && ((hevc->ignore_bufmgr_error & 0x1) == 0))
		return 1;
	return 0;
}
...
static int get_pic_poc(struct hevc_state_s *hevc,
					   unsigned int idx)
{
	if (idx != 0xff && idx < MAX_REF_PIC_NUM && hevc->m_PIC[idx])
...
static void restore_decode_state(struct hevc_state_s *hevc)
{
	...
	hevc_print(hevc, PRINT_FLAG_VDEC_STATUS,
			   ...
			   __func__, hevc->decoding_pic ? hevc->decoding_pic->index : 0xff);
						  ...
						  uint16_t *blk8x4Cb, uint16_t *blk8x4Cr, uint16_t *cmBodyBuf, int32_t direction)
{
	if (direction == 0)
	{
		...
		blk8x4Luma[3 + 1 * 8] = ((cmBodyBuf[1] << 6) | (cmBodyBuf[0] >> 10)) & 0x3ff;
		blk8x4Luma[3 + 2 * 8] = ((cmBodyBuf[1] >> 4)) & 0x3ff;
		blk8x4Luma[3 + 3 * 8] = ((cmBodyBuf[2] << 2) | (cmBodyBuf[1] >> 14)) & 0x3ff;
		blk8x4Luma[7 + 0 * 8] = ((cmBodyBuf[3] << 8) | (cmBodyBuf[2] >> 8)) & 0x3ff;
		blk8x4Luma[7 + 1 * 8] = ((cmBodyBuf[3] >> 2)) & 0x3ff;
		blk8x4Luma[7 + 2 * 8] = ((cmBodyBuf[4] << 4) | (cmBodyBuf[3] >> 12)) & 0x3ff;
		blk8x4Luma[7 + 3 * 8] = ((cmBodyBuf[4] >> 6)) & 0x3ff;
		blk8x4Cb[0 + 0 * 4] = ((cmBodyBuf[5] >> 0)) & 0x3ff;
		blk8x4Cr[0 + 0 * 4] = ((cmBodyBuf[6] << 6) | (cmBodyBuf[5] >> 10)) & 0x3ff;
		blk8x4Cb[0 + 1 * 4] = ((cmBodyBuf[6] >> 4)) & 0x3ff;
		blk8x4Cr[0 + 1 * 4] = ((cmBodyBuf[7] << 2) | (cmBodyBuf[6] >> 14)) & 0x3ff;
...
static void delrefill(struct hevc_state_s *hevc)
{
	...
	if (detResult)
	{
		...
		for (errorIdx = 0; errorIdx < errorNum; errorIdx++)
		{
			...
			uint16_t ctux = hevc->m_tile[0][tilex].start_cu_x + hevc->m_tile[0][tilex].width - 1;
...
static int get_mv_buf(struct hevc_state_s *hevc, struct PIC_s *pic)
{
	...
	if (pic && pic->index >= 0)
	{
		...
		pic->mpred_mv_wr_start_addr =
			hevc->work_space_buf->mpred_mv.buf_start + (pic->index * mv_size);
		pic->mv_size = mv_size;
	}
	return 0;
#else
	...
	{
		int MV_MEM_UNIT =
			hevc->lcu_size_log2 == 6 ? 0x200 : hevc->lcu_size_log2 == 5 ? 0x80
																		: 0x20;
		int extended_pic_width = (pic->width + hevc->lcu_size - 1) & (~(hevc->lcu_size - 1));
		int extended_pic_height = (pic->height + hevc->lcu_size - 1) & (~(hevc->lcu_size - 1));
								...
								struct buf_stru_s *buf_stru)
{
	...
	int pic_height_lcu = (pic_height % lcu_size) ? pic_height / lcu_size +
												 ...
												 : pic_height / lcu_size;
	/*SUPPORT_10BIT*/
	int losless_comp_header_size = compute_losless_comp_header_size(pic_width, pic_height);
	/*always alloc buf for 10bit*/
	int losless_comp_body_size = compute_losless_comp_body_size(hevc, pic_width, pic_height, 0);
	...
	if (dw_mode)
	{
		...
		int pic_width_lcu_dw = (pic_width_dw % lcu_size) ? pic_width_dw / lcu_size + 1 : pic_width_dw / lcu_size;
		int pic_height_lcu_dw = (pic_height_dw % lcu_size) ? pic_height_dw / lcu_size + 1 : pic_height_dw / lcu_size;
...
static int get_work_pic_num(struct hevc_state_s *hevc)
{
	...
	if (get_dynamic_buf_num_margin(hevc) > 0)
	{
			...
			(hevc->param.p.sps_max_dec_pic_buffering_minus1_0))
		{
			...
			used_buf_num = hevc->sps_num_reorder_pics_0 + get_dynamic_buf_num_margin(hevc);

		sps_pic_buf_diff = hevc->param.p.sps_max_dec_pic_buffering_minus1_0 - hevc->sps_num_reorder_pics_0;
#ifdef MULTI_INSTANCE_SUPPORT
		/*
...
static int v4l_parser_work_pic_num(struct hevc_state_s *hevc)
{
	...
	if (get_dynamic_buf_num_margin(hevc) > 0)
	{
			...
			(hevc->param.p.sps_max_dec_pic_buffering_minus1_0))
		{
			...
			used_buf_num = hevc->param.p.sps_num_reorder_pics_0 + get_dynamic_buf_num_margin(hevc);

		sps_pic_buf_diff = hevc->param.p.sps_max_dec_pic_buffering_minus1_0 - hevc->param.p.sps_num_reorder_pics_0;
#ifdef MULTI_INSTANCE_SUPPORT
		/*
		need one more for multi instance, as
		apply_ref_pic_set() has no chanch to run to
			...
			if (get_cpu_major_id() >= AM_MESON_CPU_MAJOR_ID_GXL)
			{
				...
				WRITE_VREG(HEVCD_MPP_ANC2AXI_TBL_CMD_ADDR,
						   hevc->m_PIC[i]->mc_u_v_adr |
							   (hevc->m_PIC[i]->mc_canvas_u_v << 8) | 0x1);
		}
	}
	if (cur_pic_num == 0)
		return;
								...
								unsigned char flush_flag)
{
	...
	else
	{
			...
			if (pic->output_mark)
			{
				if (pic_display)
				{
					if (pic->POC < pic_display->POC)
						pic_display = pic;
					else if ((pic->POC == pic_display->POC) && (pic->decode_idx <
																pic_display->decode_idx))
						pic_display = pic;
				}
				else
		...
		if ((vdec->master || vdec->slave) && (pic_display == hevc->cur_pic) &&
			(!flush_flag) &&
			(hevc->bypass_dvenl && !dolby_meta_with_el) && (!fisrt_pic_flag))
			pic_display = NULL;
#endif
		...
		for (i = 0; i < cur_pic->RefNum_L0; i++)
		{
			...
			if (pic)
			{
				...
				WRITE_VREG(HEVCD_MPP_ANC_CANVAS_DATA_ADDR,
						   (pic->mc_canvas_u_v << 16) | (pic->mc_canvas_u_v << 8) |
							   pic->mc_canvas_y);
				if (get_dbg_flag(hevc) & H265_DEBUG_BUFMGR)
				{
					hevc_print_cont(hevc, 0,
...
static unsigned char is_ref_long_term(struct hevc_state_s *hevc, int poc)
{
	...
	for (ii = 0; ii < MAX_REF_PIC_NUM; ii++)
	{
		...
		if (pic->referenced && pic->POC == poc && pic->long_term_ref)
			return 1;
	}
	return 0;
}
...
static void set_ref_pic_list(struct hevc_state_s *hevc, union param_u *params)
{
			...
			for (rIdx = 0; rIdx < num_ref_idx_l0_active; rIdx++)
			{
...
#ifdef SUPPORT_LONG_TERM_RPS
					cIdx >= (num_neg + num_pos) ? RefPicSetLtCurr[cIdx - num_neg - num_pos] :
#endif
												(cIdx >=
														 num_neg
				...
				if (get_dbg_flag(hevc) & H265_DEBUG_BUFMGR)
				{
					hevc_print_cont(hevc, 0, "%d ",
									pic->m_aiRefPOCList0[pic->slice_idx]
														[rIdx]);
				}
			}
		}
		...
		if (params->p.slice_type == B_SLICE)
		{
			if (params->p.modification_flag & 0x2)
			{
					 ...
					 rIdx++)
				{
					...
					{
						hevc_print_cont(hevc, 0, "%d ",
										pic->m_aiRefPOCList1[pic->slice_idx]
															[rIdx]);
					}
				}
			}
	...
	/*set m_PIC */
	pic->slice_type = (params->p.slice_type == I_SLICE) ? 2 : (params->p.slice_type == P_SLICE) ? 1
														  : (params->p.slice_type == B_SLICE)	? 0
																								: 3;
							 ...
							 union param_u *params)
{
	...
	{
		int tmpRefillLcuSize = 1 << (params->p.log2_min_coding_block_size_minus3 +
	...
	if (params->p.tiles_enabled_flag & 1)
	{
		...
		if (params->p.tiles_enabled_flag & 2)
		{ /* uniform flag */
								 ...
								 int decode_pic_begin, int decode_pic_num)
{
	...
	data32 &= 0x03ffffff;
	data32 = data32 | (3 << 29) | (2 << 26) | (1 << 24) | /* stream_buffer_empty_int_amrisc_enable */
 ...
 */
	...
	if ((misc_flag0 & (1 << DEBLOCKING_FILTER_OVERRIDE_ENABLED_FLAG_BIT)) && (misc_flag0 & (1 << DEBLOCKING_FILTER_OVERRIDE_FLAG_BIT)))
	{
	...
	if ((misc_flag0 & (1 << PPS_LOOP_FILTER_ACROSS_SLICES_ENABLED_FLAG_BIT)) && ((misc_flag0 & (1 << SLICE_SAO_LUMA_FLAG_BIT)) || (misc_flag0 & (1 << SLICE_SAO_CHROMA_FLAG_BIT)) || (!slice_deblocking_filter_disabled_flag)))
	{
		data32 |=
			((misc_flag0 >>
...
static void pic_list_process(struct hevc_state_s *hevc)
{
	...
	for (i = 0; i < MAX_REF_PIC_NUM; i++)
	{
		...
		if (pic == NULL || pic->index == -1)
			continue;
		alloc_pic_count++;
		if (pic->output_mark == 0 && pic->referenced == 0 && pic->output_ready == 0 && (pic->width != hevc->pic_w || pic->height != hevc->pic_h))
		{
	...
	if (get_alloc_pic_count(hevc) != alloc_pic_count)
	{
		hevc_print_cont(hevc, 0,
								 ...
								 union param_u *rpm_param)
{
	...
	for (i = 0; i < MAX_REF_PIC_NUM; i++)
	{
		...
		if (pic->output_mark == 0 && pic->referenced == 0 && pic->output_ready == 0 && pic->width == hevc->pic_w && pic->height == hevc->pic_h && pic->vf_ref == 0)
		{
			if (new_pic)
			{
...
static void flush_output(struct hevc_state_s *hevc, struct PIC_s *pic)
{
	...
	do
	{
		...
		if (pic_display)
		{
			pic_display->referenced = 0;
			put_mv_buf(hevc, pic_display);
			if ((pic_display->error_mark && ((hevc->ignore_bufmgr_error & 0x2) == 0)) || (get_dbg_flag(hevc) & H265_DEBUG_DISPLAY_CUR_FRAME) || (get_dbg_flag(hevc) & H265_DEBUG_NO_DISPLAY))
			{
			...
			{
				if (hevc->i_only & 0x1 && pic_display->slice_type != 2)
				{
					pic_display->output_ready = 0;
				}
				else
...
#endif
	if (hevc->m_nalUnitType != NAL_UNIT_CODED_SLICE_IDR && hevc->m_nalUnitType !=
		...
		do
		{
			...
			if (pic_display)
			{
				...
				else
				{
					...
					{
						prepare_display_buf(hw_to_vdec(hevc),
											pic_display);
						if (get_dbg_flag(hevc) &
							H265_DEBUG_BUFMGR)
						{
										...
										int decoded_lcu)
{
	...
	if ((error_handle_policy & 0x20) == 0 && hevc->cur_pic != NULL)
	{
		...
		{
			if (hevc->lcu_x_num_pre != 0 && hevc->lcu_y_num_pre != 0 && current_lcu_idx != 0 && current_lcu_idx < ((hevc->lcu_x_num_pre * hevc->lcu_y_num_pre) - 1))
				hevc->cur_pic->error_mark = 1;
		}
									...
									int decoded_lcu)
{
	...
	if ((error_handle_policy & 0x20) == 0 && hevc->cur_pic != NULL)
	{
		if (hevc->lcu_x_num != 0 && hevc->lcu_y_num != 0 && current_lcu_idx != 0 && current_lcu_idx < ((hevc->lcu_x_num * hevc->lcu_y_num) - 1))
			hevc->cur_pic->error_mark = 1;
		...
		if (hevc->cur_pic->error_mark)
		{
			...
			if (((hevc->i_only & 0x4) == 0) && hevc->cur_pic->POC && (hevc->cur_pic->slice_type == 0) && ((hevc->cur_pic->POC + MAX_BUF_NUM) < hevc->iPrevPOC))
			{
				hevc_print(hevc, 0,
											 ...
											 int decode_pic_begin)
{
	...
	if (hevc->wait_buf == 0)
	{
		...
		{
			if (hevc->curr_pic_struct == 0 || hevc->curr_pic_struct == 7 || hevc->curr_pic_struct == 8)
		...
		hevc->slice_type = (rpm_param->p.slice_type == I_SLICE) ? 2 : (rpm_param->p.slice_type == P_SLICE) ? 1
																  : (rpm_param->p.slice_type == B_SLICE)   ? 0
																										   : 3;
		...
		if (hevc->pic_w != rpm_param->p.pic_width_in_luma_samples || hevc->pic_h !=
																		 rpm_param->p.pic_height_in_luma_samples)
		{
			hevc_print(hevc, 0,
		...
		if (hevc->bit_depth_chroma > 10 ||
			...
			((get_cpu_major_id() == AM_MESON_CPU_MAJOR_ID_M8B) &&
			 (hevc->bit_depth_chroma > 8 || hevc->bit_depth_luma > 8)))
		{
		...
		else
		{
			...
			if ((iPOClsb < iPrevPOClsb) && ((iPrevPOClsb - iPOClsb) >=
											(iMaxPOClsb / 2)))
				iPOCmsb = iPrevPOCmsb + iMaxPOClsb;
			else if ((iPOClsb > iPrevPOClsb) && ((iPOClsb - iPrevPOClsb) >
												 (iMaxPOClsb / 2)))
				iPOCmsb = iPrevPOCmsb - iMaxPOClsb;
			else
				iPOCmsb = iPrevPOCmsb;
			...
			if (hevc->m_nalUnitType == NAL_UNIT_CODED_SLICE_BLA || hevc->m_nalUnitType == NAL_UNIT_CODED_SLICE_BLANT || hevc->m_nalUnitType == NAL_UNIT_CODED_SLICE_BLA_N_LP)
			{
		...
		if (hevc->m_pocRandomAccess == MAX_INT)
		{ /* first picture */
		...
		if (rpm_param->p.slice_segment_address == 0)
		{
			...
			/* new pic */
			hevc->cur_pic = hevc->is_used_v4l ? v4l_get_new_pic(hevc, rpm_param) : get_new_pic(hevc, rpm_param);
	...
	if (hevc->new_pic)
	{
		...
		/*SUPPORT_10BIT*/
		int sao_mem_unit =
			(hevc->lcu_size == 16 ? 9 : hevc->lcu_size == 32 ? 14
															 : 24)
			<< 4;
#else
		...
		hevc->tile_y_x =
			get_tile_index(hevc, rpm_param->p.slice_segment_address,
						   (hevc->pic_w +
							...
							1) /
							   hevc->lcu_size);
		if ((hevc->tile_y_x != (hevc->tile_x | (hevc->tile_y << 8))) && (hevc->tile_y_x != -1))
		{
			hevc->new_tile = 1;
	...
	if ((hevc->tile_x > (MAX_TILE_COL_NUM - 1)) || (hevc->tile_y > (MAX_TILE_ROW_NUM - 1)))
		hevc->new_tile = 0;
	...
	if (hevc->slice_type != 2)
	{
		...
		if (hevc->Col_POC != INVALID_POC)
		{
			...
			{
				if ((hevc->col_pic->width != hevc->pic_w) ||
					(hevc->col_pic->height != hevc->pic_h))
				{
					hevc_print(hevc, 0,
	...
	return 0xf;
#else
	if ((decode_pic_begin > 0 && hevc->decode_idx <= decode_pic_begin) || (dbg_skip_flag))
		return 0xf;
#endif
...
static int hevc_local_init(struct hevc_state_s *hevc)
{
	...
	else
	{
		...
		{ // get_cpu_major_id() > AM_MESON_CPU_MAJOR_ID_TM2 || is_cpu_tm2_revb()
			if (vdec_is_support_4k())
			{
...
static void set_canvas(struct hevc_state_s *hevc, struct PIC_s *pic)
{
	...
	if (get_cpu_major_id() > AM_MESON_CPU_MAJOR_ID_M8B)
		cfg_endian = hevc->is_used_v4l ? 0 : 7;
	else
		cfg_endian = 0;
	...
	if (pic->double_write_mode)
	{
		...
		canvas_config_ex(pic->y_canvas_index,
						 ...
						 CANVAS_ADDR_NOWRAP, blkmode, cfg_endian);
		...
		pic->canvas_config[0].block_mode =
			blkmode;
		pic->canvas_config[0].endian = cfg_endian;
		...
		pic->canvas_config[1].block_mode =
			blkmode;
		pic->canvas_config[1].endian = cfg_endian;
					 ...
					 struct PIC_s *pic, char *sei_buf, uint32_t size)
{
	...
	if ((nal_unit_type != NAL_UNIT_SEI) && (nal_unit_type != NAL_UNIT_SEI_SUFFIX))
		return 0;
	while (p + 4 <= sei_buf + size)
	{
		...
		if (p + payload_size <= sei_buf + size)
		{
			...
			case SEI_UserDataITU_T_T35:
				p_sei = p;
				if (p_sei[0] == 0xB5 && p_sei[1] == 0x00 && p_sei[2] == 0x3C && p_sei[3] == 0x00 && p_sei[4] == 0x01 && p_sei[5] == 0x04)
				{
					char *new_buf;
				...
				else if (p_sei[0] == 0x26 && p_sei[1] == 0x00 && p_sei[2] == 0x04 && p_sei[3] == 0x00 && p_sei[4] == 0x05)
				{
			...
			case SEI_MasteringDisplayColorVolume:
				...
				for (i = 0; i < 2; i++)
				{
					hevc->luminance[i] = (*p_sei << 24) | (*(p_sei + 1) << 16) | (*(p_sei + 2) << 8) | *(p_sei + 3);
					p_sei += 4;
				}
...
void ateme_parse_sei(struct hevc_state_s *hevc, unsigned char *input, size_t input_len)
{
	...
	if (remaining_len < sei_mdcv_header_len + sei_mdcv_data_len)
		return;
	...
	if (remaining_len < sei_cll_header_len + sei_cll_data_len)
		return;

	unsigned char *sei_cll_pos = sei_mdcv_pos + sei_mdcv_header_len + sei_mdcv_data_len;

	// Next must be the sei cll.
	if (memcmp(sei_cll_pos, sei_cll_header, sei_cll_header_len) != 0)
		return;
						   ...
						   struct PIC_s *pic)
{
	...
	unsigned size = 0;
	unsigned type = 0;
	struct vframe_master_display_colour_s *vf_dp = &vf->prop.master_display_colour;
	...
	*/
	...
	if (pic->aux_data_buf && pic->aux_data_size)
	{
		/* parser sei */
		p = pic->aux_data_buf;
		while (p < pic->aux_data_buf + pic->aux_data_size - 8)
		{
			size = *p++;
			size = (size << 8) | *p++;
			size = (size << 8) | *p++;
	...
	if (hevc->sei_present_flag & SEI_MASTER_DISPLAY_COLOR_MASK)
	{
		...
		for (i = 0; i < 2; i++)
		{
			...
			vf_dp->luminance[i] = hevc->luminance[i];
		}
	...
	{
		vf_dp->content_light_level.max_content = hevc->content_light_level[0];
		vf_dp->content_light_level.max_pic_average = hevc->content_light_level[1];
	...
	if ((hevc->sei_present_flag & SEI_HDR10PLUS_MASK) && (pic->hdr10p_data_buf != NULL) && (pic->hdr10p_data_size != 0))
	{
		char *new_buf;
...
static void vh265_vf_put(struct vframe_s *vf, void *op_arg)
{
	...
	if (index_top != 0xff && index_top < MAX_REF_PIC_NUM && hevc->m_PIC[index_top])
	{
		if (hevc->is_used_v4l)
	...
	if (index_bot != 0xff && index_bot < MAX_REF_PIC_NUM && hevc->m_PIC[index_bot])
	{
		if (hevc->is_used_v4l)
...
static int vh265_event_cb(int type, void *data, void *op_arg)
{
	...
	else if (type & VFRAME_EVENT_RECEIVER_GET_AUX_DATA)
	{
		...
		if (index != 0xff && index < MAX_REF_PIC_NUM && hevc->m_PIC[index])
		{
...
static inline void hevc_update_gvs(struct hevc_state_s *hevc, struct PIC_s *pic)
{
	...
	if (hevc->gvs->frame_dur != hevc->frame_dur)
	{
		...
		if (hevc->frame_dur != 0)
			hevc->gvs->frame_rate = ((96000 * 10 / hevc->frame_dur) % 10) < 5 ? 96000 / hevc->frame_dur : (96000 / hevc->frame_dur + 1);
		else
...
static int post_video_frame(struct vdec_s *vdec, struct PIC_s *pic)
{
	...
	if (vf)
	{
		...
		if (hevc->is_used_v4l)
		{
			vf->v4l_mem_handle = hevc->m_BUF[pic->BUF_index].v4l_ref_buf_addr;
			if (hevc->mmu_enable)
			{
		...
		else
		{
			...
			{
				if (pts_lookup_offset_us64(PTS_TYPE_VIDEO, stream_offset, &vf->pts,
		...
		else
		{
			...
			else
			{
				...
				vf->compHeadAddr = pic->mc_y_adr +
								   pic->losless_comp_body_size; /*head adr*/
				vf->mem_head_handle = NULL;
			}
			vf->canvas0Addr = vf->canvas1Addr = 0;
		}
...
#ifdef MULTI_INSTANCE_SUPPORT
			if (hevc->m_ins_flag &&
				(get_dbg_flag(hevc) & H265_CFG_CANVAS_IN_DECODE) == 0)
			{
				vf->canvas0Addr = vf->canvas1Addr = -1;
				vf->plane_num = 2;
				vf->canvas0_config[0] =
...
static int notify_v4l_eos(struct vdec_s *vdec)
{
	...
	if (hw->eos)
	{
		...
		vf->v4l_mem_handle = (index == INVALID_IDX) ? (ulong)fb : hw->m_BUF[index].v4l_ref_buf_addr;
		vdec_vframe_ready(vdec, vf);
...
#define ES_VID_MAN_RD_PTR (1 << 0)
	...
	WRITE_VREG(DOS_SW_RESET3,
			   /* (1<<2)| */
			   (1 << 3) | (1 << 4) | (1 << 8) |
				   (1 << 11) | (1 << 12) | (1 << 14) | (1 << 15) | (1 << 17) | (1 << 18) | (1 << 19));
...
#ifdef SWAP_HEVC_UCODE
	if (!vdec_tee_enabled() && hevc->is_swap &&
	...
	else if (dec_status == HEVC_DECPIC_DATA_DONE)
	{
		...
		if (hevc->m_ins_flag)
		{
			...
			if (hevc->is_swap &&
				get_cpu_major_id() <= AM_MESON_CPU_MAJOR_ID_GXM)
			{
				if (hevc->detbuf_adr_virt && hevc->delrefill_check && READ_VREG(HEVC_SAO_DBG_MODE0))
		...
		pic_done:
			if (input_frame_based(hw_to_vdec(hevc)) &&
				...
				(hevc->decode_size - (READ_VREG(HEVC_SHIFT_BYTE_COUNT)) > frmbase_cont_bitlevel))
			{
				...
				hevc->ip_mode)
			{
				...
				check_pic_decoded_error(hevc,
										READ_VREG(HEVC_PARSER_LCU_START) & 0xffffff);
				if (hevc->cur_pic != NULL &&
					(READ_VREG(HEVC_PARSER_LCU_START) & 0xffffff) == 0 && (hevc->lcu_x_num * hevc->lcu_y_num != 1))
					hevc->cur_pic->error_mark = 1;
#ifdef CONFIG_AMLOGIC_MEDIA_ENHANCEMENT_DOLBYVISION
			force_output:
#endif
				...
				if (pic_display)
				{
					...
					{
						if ((pic_display->slice_type != 2) && !pic_display->ip_mode)
						{
							pic_display->output_ready = 0;
						}
						else
			...
			else if (hevc->have_sps && hevc->have_pps)
			{
					 ...
					 NAL_UNIT_CODED_SLICE_BLA_N_LP))
				{
					...
					hevc->have_valid_start_slice = 1;
							 ...
							 NAL_UNIT_CODED_SLICE_CRA &&
						 (hevc->have_valid_start_slice || (hevc->PB_skip_mode != 3)))
				{
					if (slice_parse_begin > 0)
					{
						hevc_print(hevc, 0,
	...
	else if (dec_status == HEVC_SLICE_SEGMENT_DONE)
	{
		...
		else if (hevc->wait_buf == 0)
		{
			...
			if (hevc->video_signal_type !=
				((hevc->param.p.video_signal_type << 16) | hevc->param.p.color_description))
			{
			...
			if (use_cma &&
				(hevc->param.p.slice_segment_address == 0) && (hevc->pic_list_init_flag == 0))
			{
				...
				if (hevc->pic_w == 0 || hevc->pic_h == 0 || hevc->lcu_size == 0 || is_oversize(hevc->pic_w, hevc->pic_h) || hevc_skip_nal(hevc))
				{
					/* skip search next start code */
					WRITE_VREG(HEVC_WAIT_FLAG, READ_VREG(HEVC_WAIT_FLAG) & (~0x2));
					if ((hevc->pic_h == 96) && (hevc->pic_w == 160))
				...
				else
				{
					...
					if ((!IS_4K_SIZE(hevc->pic_w, hevc->pic_h)) &&
						((hevc->param.p.profile_etc & 0xc) == 0x4) && (interlace_enable != 0))
					{
...
static void vh265_check_timer_func(unsigned long arg)
{
	...
	else
	{
		...
		/* error watchdog */
		if (hevc->m_ins_flag == 0 &&
			(empty_flag == 0) && (hevc->pic_list_init_flag == 0 || hevc->pic_list_init_flag == 3))
		{
			/* decoder has input */
			if ((get_dbg_flag(hevc) &
				 H265_DEBUG_DIS_LOC_ERROR_PROC) == 0)
			...
			if ((get_dbg_flag(hevc) & H265_DEBUG_DIS_SYS_ERROR_PROC) == 0)
				/* receiver has no buffer to recycle */
				if ((state == RECEIVER_INACTIVE) &&
					(kfifo_is_empty(&hevc->display_q)))
	...
	if (hevc->m_ins_flag == 0 &&
		...
		hevc->frame_dur > 0 && hevc->saved_resolution != hevc->frame_width * hevc->frame_height * (96000 / hevc->frame_dur))
...
#endif
{
	...
	if (hevc->frame_dur != 0)
		vstatus->frame_rate = ((96000 * 10 / hevc->frame_dur) % 10) < 5 ? 96000 / hevc->frame_dur : (96000 / hevc->frame_dur + 1);
	else
...
static void config_decode_mode(struct hevc_state_s *hevc)
{
	...
	else if (vdec->slave)
	{
		...
		if (hevc->bypass_dvenl)
			decode_mode =
				(hevc->start_parser_type << 8) | DECODE_MODE_MULTI_STREAMBASE;
		else
			decode_mode =
				(hevc->start_parser_type << 8) | DECODE_MODE_MULTI_DVBAL;
	}
	else if (vdec->master)
		decode_mode =
			(hevc->start_parser_type << 8) | DECODE_MODE_MULTI_DVENL;
#endif
	else
		decode_mode =
...
static unsigned char get_data_check_sum(struct hevc_state_s *hevc, int size)
{
	int jj;
	int sum = 0;
	u8 *data = NULL;
								 ...
								 struct vdec_s *vdec, int from)
{
	 ...
	 */
	if (hevc->pic_list_init_flag == 1 && (hevc->dec_result != DEC_RESULT_FORCE_EXIT))
	{
		hevc->pic_list_init_flag = 2;
		init_pic_list(hevc);
		init_pic_list_hw(hevc);
		 ...
		 VDEC_STATUS_DISCONNECTED))
	{
		...
		if (!input_frame_based(vdec))
		{
			...
			if (r >= 0x200)
			{
				...
				start_process_time(hevc);
				if (READ_VREG(HEVC_DEC_STATUS_REG) == HEVC_DECODE_BUFEMPTY2)
					WRITE_VREG(HEVC_DEC_STATUS_REG,
							   HEVC_ACTION_DONE);
				else
					WRITE_VREG(HEVC_DEC_STATUS_REG,
		...
		if (run_ready(vdec, VDEC_HEVC))
		{
			...
			hevc_print(hevc, PRINT_FLAG_VDEC_STATUS,
					   ...
					   __func__, r,
					   (get_dbg_flag(hevc) & PRINT_FLAG_VDEC_STATUS) ? get_data_check_sum(hevc, r) : 0,
					   READ_VREG(HEVC_MPC_E));
...
static unsigned long run_ready(struct vdec_s *vdec, unsigned long mask)
{
	struct hevc_state_s *hevc =
		(struct hevc_state_s *)vdec->private;
	int tvp = vdec_secure(hw_to_vdec(hevc)) ? CODEC_MM_FLAGS_TVP : 0;
	bool ret = 0;
#ifdef VDEC_FCC_SUPPORT
	int get_msg = 1;
#endif
	...
	if (vdec_stream_based(vdec) && (hevc->init_flag == 0) && pre_decode_buf_level != 0)
	{
		u32 rp, wp, level;
				...
				void (*callback)(struct vdec_s *, void *), void *arg)
{
	...
	hevc->dec_result = DEC_RESULT_NONE;
	if (vdec_frame_based(vdec) &&
		((get_dbg_flag(hevc) & PRINT_FLAG_VDEC_STATUS) || is_log_enable(hevc)) &&
		!vdec_secure(vdec))
	...
	else
	{
		...
		if (loadr < 0)
		{
			...
			hevc_print(hevc, 0, "H265: the %s fw loading failed, err: %x\n",
					   vdec_tee_enabled() ? "TEE" : "local", loadr);
			hevc->dec_result = DEC_RESULT_FORCE_EXIT;
			vdec_schedule_work(&hevc->work);
			return;
		}
...
static int amvdec_h265_probe(struct platform_device *pdev)
{
	...
	{
		if (get_cpu_major_id() < AM_MESON_CPU_MAJOR_ID_GXL || double_write_mode == 0x10)
			hevc->mmu_enable = 0;
		else
			hevc->mmu_enable = 1;
	}
...
static int ammvdec_h265_probe(struct platform_device *pdev)
{
	...
	else if (vdec_dual(pdata))
	{
		...
		if (dv_toggle_prov_name) /*debug purpose*/
			snprintf(pdata->vf_provider_name,
					 VDEC_PROVIDER_NAME_SIZE,
					 (pdata->master) ? VFM_DEC_DVBL_PROVIDER_NAME : VFM_DEC_DVEL_PROVIDER_NAME);
		else
			snprintf(pdata->vf_provider_name,
					 VDEC_PROVIDER_NAME_SIZE,
					 (pdata->master) ? VFM_DEC_DVEL_PROVIDER_NAME : VFM_DEC_DVBL_PROVIDER_NAME);
	...
	if (pdata->parallel_dec == 1)
		...
		vdec_core_request(pdata, CORE_MASK_VDEC_1 | CORE_MASK_HEVC | CORE_MASK_COMBINE);
...
{
	struct hevc_state_s *hevc =
		(struct hevc_state_s *)(((struct vdec_s *)(platform_get_drvdata(pdev)))->private);
	struct vdec_s *vdec;
...
static int __init amvdec_h265_driver_init_module(void)
{
	...
	work_buf_size =
		(p_buf_info->end_adr - p_buf_info->start_adr + 0xffff) & (~0xffff);
...
module_param(hacked_lowlatency, uint, 0664);
MODULE_PARM_DESC(hacked_lowlatency, "\n detect_stuck_buffer_margin\n");
